...
► New in this version
    • Adds dummy `sql_ids` / `sql_mask` so downstream code that expects
      them always finds them.
    • Adds scalar latency target `data.y` = mean of runtime_list (ms).
"""
